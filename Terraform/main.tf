...
# First route table with Internet gateway connectivity
...
# Second route table with NAT connectivity
...
module "private_rds" {
  ...
  db_name           = "myDB
  db_username       = "krutikadb"
  db_password       = var.db_password     # Password is NOT commited to repo, an example file is uploaded with no password
  vpc_sg_ids        = [module.private_rds_sg.sg_id]
  subnet_group_name = "krutika_rds_subnet_group"
  subnet_ids        = [module.private_subnet_1.subnet_id, module.private_subnet_2.subnet_id]
  rds_tags = merge(
    {
      "Name"       = format("%s_private_rds", var.name)
      "dependency" = module.gateways.nat_dependency        # Create dependency on NAT 
    },
    var.tags,
  )
